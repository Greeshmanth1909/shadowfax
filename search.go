...
func IsRepetition(brd *board.S_Board) bool {
	...
	// Fix bounds checking to prevent negative index access
	startIndex := brd.HisPly - brd.FiftyMove
	if startIndex < 0 {
		startIndex = 0
	}

	for i := startIndex; i < brd.HisPly; i++ {
...
func SearchPositions(brd *board.S_Board, info *board.S_SearchInfo) {
	...
	for currentDepth = 1; currentDepth < info.Depth; currentDepth++ {
		...
		timeMs := time.Since(info.StartTime).Milliseconds()
		var nps int64
		if timeMs > 0 {
			nps = (int64(info.Nodes) * 1000) / timeMs
		} else {
			nps = 0
		}

		fmt.Printf("info score cp %v depth %v nodes %v time %v nps %v pv", bestScore, currentDepth, info.Nodes, timeMs, nps)
		for i := 0; i < pvMoves; i++ {
			val := brd.PvArray[i]
			var mv eval.S_Move
			mv.Move = val
